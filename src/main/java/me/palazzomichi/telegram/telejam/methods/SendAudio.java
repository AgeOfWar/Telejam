...
public class SendAudio implements TelegramMethod<AudioMessage> {
  ...
  /**
   * True if the audio is not present in Telegram servers.
   */
  private boolean newAudio;
  ...
  public SendAudio audio(File audio) {
    newAudio = true;
    this.audio = audio.getPath();
    return this;
  }

  public SendAudio audio(Path audio) {
    newAudio = true;
  ...
  public HttpEntity getHttpEntity() {
    if (newAudio) {
      ...
      if (audio != null)
        builder.addBinaryBody(AUDIO_FIELD, new File(audio));
